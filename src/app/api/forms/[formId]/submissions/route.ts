...
export async function POST(request: Request, { params }: RouteContext) {
  ...
  try {
    ...
    const { submissionBlobId } = await storeSubmission(
      ...
      form.sealPolicyId ?? undefined,
    )

    return NextResponse.json(
      apiSuccess({ submissionId: payload.id, submissionBlobId }),
      { status: 201 },
    )
  } catch (err) {
    ...
    console.error(
      `[API] POST /api/forms/${formId}/submissions unexpected error:`,
      err,
    )
    return NextResponse.json(
...
export async function GET(request: Request, { params }: RouteContext) {
  ...
  try {
    ...
    console.error(
      `[API] GET /api/forms/${formId}/submissions unexpected error:`,
      err,
    )
    return NextResponse.json(
      apiError('INTERNAL_ERROR', 'An unexpected error occurred.'),
