...
import { parseTypeformExport } from '@/lib/import/typeform'
...
export async function POST(request: Request) {
  ...
  // ── Parse Typeform export ─────────────────────────────────────────────────
  let result: ReturnType<typeof parseTypeformExport>
  try {
    result = parseTypeformExport(body)
  } catch (err) {
    const message =
      err instanceof Error ? err.message : 'Failed to parse Typeform export.'
  ...
  try {
    ...
    console.error('[API] Typeform import — draft form creation failed:', err)
    return NextResponse.json(
